...
class LightFM(object):
    ...
    def __init__(
        ...
        assert loss in ("custom", "logistic", "warp", "bpr", "warp-kos")
    ...
    def _run_epoch(
        ...
        if loss in ("custom", "warp", "bpr", "warp-kos"):
        ...
        if loss == "custom":
            if user_profiles.dtype != CYTHON_DTYPE:
                user_profiles = np.ascontiguousarray(user_profiles.astype(CYTHON_DTYPE))
            if item_profiles.dtype != CYTHON_DTYPE:
                item_profiles = np.ascontiguousarray(item_profiles.astype(CYTHON_DTYPE))
            fit_custom(
